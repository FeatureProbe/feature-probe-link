...
impl RegistryNode {
    pub fn new<T: Into<String>>(addr: T, use_ssl: bool) -> Self {
        let addr = addr.into();
        Self {
            listen_addr: addr,
            use_ssl,
...
impl ServiceNode {
    pub fn from_fplink<T: Into<String>>(service_id: T, node: ServerNode) -> Self {
        Self::new(
            service_id.into(),
            node.node_id,
            node.zone,
...
mod tests {
    ...
    #[test]
    fn test_deserlize() {
        let string = "{\"serviceId\":\"service\",\"host\":\"10.96.102.109\",\"port\":8080,\"secure\":false,\"uri\":null,\"scheme\":\"http\",\"instanceId\":\"10.96.102.109\", \"unknown_field\": \"\"}";
        let node: ServiceNode = serde_json::from_str(string).unwrap();
        assert_eq!(node.service_id, "service".to_owned());
